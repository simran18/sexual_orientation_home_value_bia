...
dat <- read.csv(file = 'Allcases_comp.csv', header=T)
...
```{r fast participant drop count and rate (% sample)}

# Drop rate (% of participants dropped for overly fast responding)

clean$fastprt.count
clean$fastprt.rate
clean2$fastprt.count
clean2$fastprt.rate
clean3$fastprt.count
clean3$fastprt.rate
...
write.csv(data.frame(dscore = dscore), "IAToutputAllCases_D1.csv")

dscore2[is.na(dscore2)] <- -99 
write.csv(data.frame(dscore2 = dscore2), "IAToutputAllCases_D2.csv")

dscore3[is.na(dscore3)] <- -99 
write.csv(data.frame(dscore3 = dscore3), "IAToutputAllCases_D3.csv")
